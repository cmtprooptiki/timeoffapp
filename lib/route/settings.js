...
router.post('/leavetypes', function(req, res){
  ...
  req.user
    ...
    .catch(error => {
      ...
      req.session.flash_error(
        'Αποτυχία ενημέρωσης τύπων άδειας'
      );
...
router.post('/leavetypes/delete/:leave_type_id/', function(req, res){
    ...
    if (!validator.isInt(leave_type_id)) {
        ...
        req.session.flash_error('Αδύνατη η διαγραφή τύπου άδειας: λάθος παράμετροι');
    ...
    .then(function(company){
        ...
        // Check if there exist leaves for current type and if so, do not remove it
            ...
            req.session.flash_error('Αδύνατη η διαγραφή: ο τύπος άδειας χρησιμοποιείται');
    ...
    .then(function(){
        req.session.flash_message('Ο τύπος άδειας διαγράφηκε επιτυχώς');
        return res.redirect_with_session('/settings/general/');
    })
    .catch(function(error){
        console.error(
        ...
        req.session.flash_error(
          'Αποτυχία διαγραφής τύπου άδειας'
        );
...
router.post('/company/integration-api/', (req, res) => {
  ...
  action = action.then(() => {
    req.session.flash_message('Οι ρυθμίσεις αποθηκεύτηκαν');
  ...
  req.user
    ...
    .then(function(){
      if ( req.session.flash_has_errors() ) {
        ...
        req.session.flash_message('Η διαμόρφωση LDAP ενημερώθηκε');
        return res.redirect_with_session('/settings/company/authentication/');
      }
    })
    .catch(function(error){
...
router.post('/company/delete/', (req, res) => {
  ...
  req.user
    ...
    .then(() => {
      req.session.flash_message(`Η εταιρεία ${company.name} και τα σχετικά δεδομένα διαγράφηκαν επιτυχώς`);
