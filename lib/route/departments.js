...
function get_and_validate_department(args) {
  ...
  if (!validator.isFloat(allowance)) {
    req.session.flash_error(
      'Ο αριθμός αδειών για '+department_name+' πρέπει να είναι αριθμός'
    );
  } else if (!((0 <= allowance) && (allowance <= 50))) {
    req.session.flash_error(
      'Ο αριθμός αδειών για '+department_name+' πρέπει να είναι μεταξύ 0.5 και 50 ημερών'
    );
  }
  ...
  if (!validator.isNumeric( boss_id ) ) {
    req.session.flash_error(
      'Η αναφορά διευθυντή για '+department_name+' πρέπει να είναι αριθμός'
    );
  } else if ( ! _.contains(
    ...
    req.session.flash_error(
      'Άγνωστος διευθυντής για '+department_name
    );
  }
...
router.post('/departments/', function(req, res){
  ...
  .then(() => {
    if ( ! req.session.flash_has_errors() ) {
      req.session.flash_message('Οι αλλαγές στα τμήματα αποθηκεύτηκαν');
    }
...
router.post('/departments/delete/:department_id/', function(req, res){
  ...
  if (!validator.isInt(department_id)) {
    ...
    req.session.flash_error('Αδύνατη η διαγραφή τμήματος: λάθος παράμετροι');
  ...
  .then(function(departments){
    ...
    if (department_to_remove.users.length > 0){
      req.session.flash_error(
        'Αδύνατη η διαγραφή του τμήματος '+department_to_remove.name+' γιατί έχει ακόμα '+department_to_remove.users.length+' χρήστες.'
      );
  ...
  .then(function(){
    req.session.flash_message('Το τμήμα διαγράφηκε επιτυχώς');
    return res.redirect_with_session('/settings/departments/');
  })
...
router.get('/departments/edit/:department_id/', function(req, res){
  ...
  .catch(function(error){
    ...
    req.session.flash_error(
      'Αποτυχία φόρτωσης λεπτομερειών τμήματος'
    );
...
router.post('/departments/edit/:department_id/', function(req, res){
  ...
  .then(function(){
    if (req.body.remove_supervisor_id) {
      ...
      .then(function(){
        req.session.flash_message('Ο επόπτης αφαιρέθηκε από το τμήμα ' + department.name);
        return Promise.resolve(1);
      });
      ...
      .then(function(){
        req.session.flash_message('Οι επόπτες προστέθηκαν στο τμήμα ' + department.name);
        return Promise.resolve(1);
      });
    }
    ...
    .then(function(){
      req.session.flash_message('Το τμήμα ' + department.name + ' ενημερώθηκε');
      return Promise.resolve(1);
    });
  })
