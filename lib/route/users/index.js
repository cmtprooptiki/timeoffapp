...
router.post('/add/', function(req, res){
  ...
  .then(function(){
    if ( req.session.flash_has_errors() ) {
      ...
      req.session.flash_message('Ο νέος λογαριασμός χρήστη προστέθηκε επιτυχώς');
  ...
  .catch(function(error){
    ...
    req.session.flash_error(
      'Αποτυχία προσθήκης νέου χρήστη'
    );
    ...
    .then(action_result => {
      ...
      if ( action_result.users.length > 0 ) {
        req.session.flash_message(
          'Οι παρακάτω χρήστες εισήχθησαν επιτυχώς: '
      ...
      if (action_result.errors.length > 0) {
        action_result.errors.forEach(err => req.session.flash_error(
          'Αποτυχία προσθήκης χρήστη ' + err.email + '. Αιτία: ' + err.error
        ));
      }
...
var ensure_email_is_not_used_elsewhere = function(args){
  ...
  return model.User
    ...
    .then(function(user){
      ...
      if (user) {
        req.session.flash_error('Το email χρησιμοποιείται ήδη');
  ...
  if (
    ...
    return model.User
      ...
      .then(function(number_of_admins_to_be_left){
        ...
        req.session.flash_error('Αυτός είναι ο τελευταίος διαχειριστής. Δεν είναι δυνατή η αφαίρεση δικαιωμάτων.');
...
router.post('/edit/:user_id/', function(req, res){
  ...
  .then(function(){
    ...
    employee
      ...
      .then(function(){
        req.session.flash_message(
          'Τα στοιχεία του/της '+employee.full_name()+' ενημερώθηκαν επιτυχώς'
        );
...
router.post('/delete/:user_id/', function(req, res){
    ...
    .then(() => {
      req.session.flash_message(
        'Τα στοιχεία του υπαλλήλου διαγράφηκαν από το σύστημα'
      );
      return res.redirect_with_session('../..');
    })
    .catch(error => {
...
router.all('/search/', function(req, res){
  ...
  if ( ! validator.isEmail( email )) {
    req.session.flash_error('Το email που δώσατε δεν φαίνεται έγκυρο: "'+email+'"');
    return res.json([]);
  }
...
function get_and_validate_user_parameters(args) {
    ...
    if (!validator.isEmail(email)) {
        req.session.flash_error(
            'Το νέο email του/της '+item_name+' πρέπει να είναι έγκυρη διεύθυνση'
        );
    }

    if (!validator.isNumeric(department_id)) {
        req.session.flash_error(
            'Ο αριθμός τμήματος του/της '+item_name+' πρέπει να είναι έγκυρος'
        );
    }

    if (adjustment && ! validator.isFloat(adjustment) ) {
      req.session.flash_error(
        'Η προσαρμογή αδειών του/της '+item_name+' πρέπει να είναι έγκυρος αριθμός'
      );
    } else if (adjustment && ! ( adjustment % 1 === 0 || Math.abs( adjustment % 1 ) === 0.5 )) {
      req.session.flash_error(
        'Η προσαρμογή αδειών του/της '+item_name+' πρέπει να είναι ακέραιος ή με .5'
      );
    }
    ...
    if (!validator.isDate(start_date)) {
      req.session.flash_error(
        'Η ημερομηνία έναρξης για '+item_name+' πρέπει να είναι έγκυρη'
      );
    }
    ...
    if (end_date ){
      ...
      if ( ! validator.isDate(end_date)) {
        req.session.flash_error(
          'Η ημερομηνία λήξης για '+item_name+' πρέπει να είναι έγκυρη'
        );
      }
    }
    ...
    if (
        ...
        req.session.flash_error(
            'Η ημερομηνία λήξης για '+item_name+' είναι πριν την ημερομηνία έναρξης'
        );
    }
    ...
    if (require_password && ! password) {
      req.session.flash_error('Απαιτείται κωδικός');
    }
