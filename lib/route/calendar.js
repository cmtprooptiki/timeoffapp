...
router.post('/bookleave/', function(req, res){
    ...
    Promise.join (
      ...
      (users, company, valid_attributes) => {
        ...
        if (!employee) {
          req.session.flash_error('Λάθος υπάλληλος');
          throw new Error( 'Got validation errors' );
        }

        if (!leave_type) {
          req.session.flash_error('Λάθος τύπος άδειας');
          throw new Error( 'Got validation errors' );
        }

        if (company.is_mode_readonly_holidays() ){
          req.session.flash_error(
            "Ο λογαριασμός είναι κλειδωμένος και δεν είναι δυνατή η προσθήκη νέων αιτημάτων άδειας. Επικοινωνήστε με τον διαχειριστή."
          );
    ...
    .then(function(){
        ...
        req.session.flash_message('Το αίτημα άδειας υποβλήθηκε επιτυχώς');
    ...
    .catch(function(error){
        ...
        req.session.flash_error('Αποτυχία δημιουργίας αιτήματος άδειας');
...
router.post('/feeds/regenerate/', function(req, res){
  ...
  req.user
    ...
    .then(function(){
        req.session.flash_message('Το feed αναδημιουργήθηκε επιτυχώς');
